...
// The returns of `UniqueUnionDeep` are expected to be equal of `UniqueUnion`, if there's no recursive object type in a passed union type.
...
// `{a: t}` isn't excluded with `{a: T}` even if `T` includes `t`.
...
// `[u, ...u]` doesn't deletes tuple of `[v, ...y]` even if `u` inclues `v`.
